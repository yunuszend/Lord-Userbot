...
async def fastpurger(purg):
    ...
    if purg.reply_to_msg_id is not None:
        ...
        return await purg.edit("`Mohon Balas Ke Pesan yah Kang Sadboy ツ `")

    if msgs:
        await purg.client.delete_messages(chat, msgs)
    done = await purg.client.send_message(
        purg.chat_id, f"`Berhasil Menghapus Pesan Kang Sadboy`\
        \nJumlah Pesan Yang Dihapus {str(count)} Pesan")
    """
    if BOTLOG:
        await purg.client.send_message(
            BOTLOG_CHATID,
            "Berhasil Menghapus Pesan Kang Sadboy " + str(count) + " Pesan Berhasil  Dibersihkan.")
    """
    await sleep(2)
    await done.delete()
...
async def purgeme(delme):
    ...
    smsg = await delme.client.send_message(
        delme.chat_id,
        "`Berhasil Menghapus Pesan dari Kang Sadboy,` " + str(count) + " `Pesan Telah Dihapus ツ`",
    )
    """
    if BOTLOG:
        await delme.client.send_message(
            BOTLOG_CHATID,
            "`Sadboy Telah Menghapus Pesan,` " + str(count) + " Pesan Telah Dihapus ツ`")
    """
    await sleep(2)
    i = 1
    await smsg.delete()
...
async def delete_it(delme):
    ...
    if delme.reply_to_msg_id:
        try:
            ...
            if BOTLOG:
                await delme.client.send_message(
                    BOTLOG_CHATID, "`Kang Sadboy Berhasil Menghapus Pesan ツ`")
            """

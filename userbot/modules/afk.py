...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"**✘ SAD!**\n**Sadboy Telah AFK**\
        \n☛ **Alasan:** `{string}`")
    else:
        await afk_e.edit("**✘ SAD!**\n**Sadboy Telah AFK**")
    if user.last_name:
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**Sadboy comeback again!**")
        time.sleep(3)
        await msg.delete()
        ...
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"**✘ Sadboy Masih meratapi nasib :(** {afk_since} **Yang Lalu.**\
                            \n☛ **Alasan:** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"✘ **Sadboy Sedang meratapi nasib :(** {afk_since} **Yang Lalu**.\
                        \n☛ **Alasan**: `{AFKREASON}`")
                else:
